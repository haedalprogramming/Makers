...
시간에 대한 처리를 하는 시계 모듈입니다. 내부 전원이 존재하여 PC나 배터리의 연결을 끊더라도 계속 모듈 내부의 시간이 흘러갑니다.    
...
`툴` - `라이브러리 관리` 로 들어가서 `라이브러리 매니저` 를 열어줍니다. 그리고 검색창에 `LiquidCrystal I2C` 라고 검색하여 `Frank de Brabander` 가 제작한 라이브러리의 버전을 선택하고 설치를 진행해줍니다. `INSTALLED` 라는 글씨가 뜨게 되면 설치가 완료된 것입니다.
...
위 링크로 들어가서 초록색 버튼으로 된 `Code` 라는 버튼를 클릭하고 `Download ZIP` 을 클릭하면 다운로드가 시작됩니다. 
...
## 코드 업로드하기   
다음 주어진 코드를 그대로 통합개발환경(IDE)에 입력합니다.
